...
import { FolderIcon, ClockIcon, CalendarIcon, ArrowLeftIcon, FolderPlusIcon, PlusIcon, ChevronRightIcon } from './Icons';
...
const LibraryView: React.FC<LibraryViewProps> = ({ onSelectVideo, onSelectFolder, onBack }) => {
  ...
  const uncategorizedVideos = videos.filter((v) => !organizedVideoIds.has(v.id));
  ...
  const handleAddToFolder = async (folderId: string, videoId: string) => {
    try {
      ...
      setFolders((prev) =>
        prev.map((f) =>
          f.id === folderId
            ? { ...f, videoIds: [...f.videoIds, videoId], videoCount: f.videoCount + 1 }
            : f
        )
      );
    ...
    <div className="bg-white rounded-xl shadow-sm border border-gray-200">
      ...
      <div className="border-b border-gray-200 p-4">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-xl font-semibold text-gray-900 flex-1">Video Library</h1>
          <button
      ...
      <div className="p-4 space-y-6">
        ...
        {showCreateFolder && (
          <div className="p-4 bg-blue-50 rounded-lg border border-blue-200">
            <h3 className="font-medium text-gray-900 mb-3">Create New Folder</h3>
        ...
        <div>
          <h2 className="text-sm font-semibold text-gray-500 uppercase tracking-wide mb-3">
            {folders.length > 0 ? `Uncategorized (${uncategorizedVideos.length})` : `All Videos (${videos.length})`}
          </h2>
          ...
          {uncategorizedVideos.length === 0 && videos.length === 0 ? (
            ...
            <p className="text-gray-500 text-sm py-4">All videos are organized in folders.</p>
          ) : (
                ...
                >
                  ...
                  {folders.length > 0 && (
                    <div className="relative">
                      <button
                        onClick={() => setAddingToFolder(addingToFolder === video.id ? null : video.id)}
                      ...
                      {addingToFolder === video.id && (
                        <div className="absolute right-0 top-full mt-1 w-48 bg-white rounded-lg shadow-lg border border-gray-200 py-1 z-10">
                          <p className="px-3 py-1 text-xs text-gray-500 uppercase">Add to folder</p>
                          {folders.map((folder) => (
                            <button
                              key={folder.id}
                              onClick={() => handleAddToFolder(folder.id, video.id)}
