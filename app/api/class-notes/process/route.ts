...
export async function POST(request: NextRequest) {
  ...
  if (!validTypes.includes(file.type)) {
    return new Response(
      JSON.stringify({ error: 'Invalid file type. Please upload MP4, WebM, MOV, or AVI.' }),
      { status: 400, headers: { 'Content-Type': 'application/json' } }
    );
  }
  ...
  if (!await fileExists(pythonPath)) {
    ...
    return new Response(
      JSON.stringify({ error: `Python virtual environment not found at ${venvDir}. Run: python3 -m venv ${venvDir} && source ${venvDir}/bin/activate && pip install python-dotenv openai anthropic` }),
  ...
  const stream = new ReadableStream({
    start(controller) {
      const pythonProcess = spawn(pythonPath, [scriptPath, tempPath, OUTPUT_DIR], {
        cwd: process.cwd(),
        env: {
          ...process.env,
      ...
      pythonProcess.on('close', async (code) => {
        ...
        if (code === 0) {
          try {
            ...
            controller.enqueue(encoder.encode(`data: ${JSON.stringify({ type: 'error', error: 'Failed to parse result' })}\n\n`));
          }
        } else {
          ...
          try {
            ...
            if (parsed.status === 'error') {
              controller.enqueue(encoder.encode(`data: ${JSON.stringify({ type: 'error', error: parsed.message })}\n\n`));
            } else {
              controller.enqueue(encoder.encode(`data: ${JSON.stringify({ type: 'error', error: `Process exited with code ${code}` })}\n\n`));
            }
          } catch {
            controller.enqueue(encoder.encode(`data: ${JSON.stringify({ type: 'error', error: `Process exited with code ${code}` })}\n\n`));
          }
      ...
      pythonProcess.on('error', async (err) => {
        await unlink(tempPath).catch(() => {});
        controller.enqueue(encoder.encode(`data: ${JSON.stringify({ type: 'error', error: `Failed to start process: ${err.message}` })}\n\n`));
        controller.close();
      });
    },
  });
